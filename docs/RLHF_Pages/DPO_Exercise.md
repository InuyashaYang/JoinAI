...
假设有一个问题序列 $x$，两个回答序列 $y_w$（较好）和 $y_l$（较差）。给定以下条件概率：

- 对于回答 $y_w$，政策模型 $\pi_\theta$ 在每个时间步 $t$ 的条件概率分别为：
  - $\pi_\theta(y_w^1|x) = 0.6$
  - $\pi_\theta(y_w^2|x, y_w^1) = 0.7$
  - $\pi_\theta(y_w^3|x, y_w^{<3}) = 0.8$

- 参考模型 $\pi_\text{ref}$ 在相同位置的条件概率分别为：
  - $\pi_\text{ref}(y_w^1|x) = 0.5$
  - $\pi_\text{ref}(y_w^2|x, y_w^1) = 0.6$
  - $\pi_\text{ref}(y_w^3|x, y_w^{<3}) = 0.7$
...
使用对数概率的性质，将每个时间步的对数概率相加。

$$
\log \pi(y|x) = \sum_{t=1}^T \log \pi(y^t|x, y^{<t})
$$

例如，对于 $\pi_\theta(y_w|x)$：

$$
\log \pi_\theta(y_w|x) = \log 0.6 + \log 0.7 + \log 0.8
$$
